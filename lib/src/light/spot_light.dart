...
/// [direction]  — world-space pointing direction (will be normalised)
class M3SpotLight extends M3PointLight {
  /// World-space direction the spotlight points toward (normalised).
  ...
  Float32List packBuffer(Matrix4 mMatrixInv) {
    ...
    final Vector4 localPos = mMatrixInv * Vector4(position.x, position.y, position.z, 1.0);

    // ----- direction in object space (w=0 → no translation) -----
    final Vector4 localDir = mMatrixInv * Vector4(direction.x, direction.y, direction.z, 0.0);
